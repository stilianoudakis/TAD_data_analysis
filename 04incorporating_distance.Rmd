...
within_mid <- (((w@first@ranges@start+w@first@ranges@width)+w@first@ranges@start)/2)
left_dist <- tad_mid-w@second@ranges@start

#Right distance
#right subcompartment interval - right tad flank
right_dist <- (w@second@ranges@start + w@second@ranges@width) - within_mid
...
logitdata$left_dist <- mcols(tad_subcomp_full_2)$left_dist
logitdata$right_dist <- mcols(tad_subcomp_full_2)$right_dist
logitdata$closest_dist <- mcols(tad_subcomp_full_2)$closest_dist

saveRDS(logitdata, "logitdata_2.rds")
```
