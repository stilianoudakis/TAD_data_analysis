...
datalist = list()
set.seed(123)
notadbins <- binslist10_center[sample(which(mcols(binslist10_center)$y==0),16770)]
for(i in 1:length(x)){
...
saveRDS("/home/stilianoudakisc/TAD_data_analysis/measuring_performances/total_feats.rds")

filtered_feats <- setdiff(names(gm12878_10kb)[-1], names(gm12878_10kb_f)[-1])
saveRDS("/home/stilianoudakisc/TAD_data_analysis/measuring_performances/filtered_feats.rds")

remaining_feats <- names(gm12878_10kb_f)[-1]
saveRDS("/home/stilianoudakisc/TAD_data_analysis/measuring_performances/remaining_feats.rds")
...
saveRDS("/home/stilianoudakisc/TAD_data_analysis/measuring_performances/perfdat.norm.rds")
...
mccf1 <- data.frame(Metric = c(rep("MCC",3), rep("F1",3)),
                    Technique = rep(c("MLR",
										"MLR w/ LASSO",
										"E-Net"), 2),
                    Value = c(as.numeric(perfdat.final[15,2:4]), as.numeric(perfdat.final[16,2:4])))
...
varimp.mlr.df <- varimp.mlr.df[(dim(varimp.mlr.df)[1]-19):dim(varimp.mlr.df)[1],]
...
varimp.mlrlasso.df <- varimp.mlrlasso.df[(dim(varimp.mlrlasso.df)[1]-19):dim(varimp.mlrlasso.df)[1],]
...
varimp.enet.df <- varimp.enet.df[(dim(varimp.enet.df)[1]-19):dim(varimp.enet.df)[1],]
