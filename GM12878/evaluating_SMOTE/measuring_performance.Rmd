...
```{r}
enetlst_sm <- readRDS("enetlst_sm.rds")
...
p<-ggplot(data=auc.sm, aes(x=Combination, y=auc, fill=Combination)) + 
  ...
  scale_fill_manual(values=rainbow(8), guide=FALSE) +
  theme_minimal() + 
...
ggplot() + 
  geom_line(aes(fpr, tpr, colour="black"), rocdat.bs) +
  geom_line(aes(fpr, tpr, colour=rainbow(8)[1]), onetwo) + 
  scale_colour_manual(name="Sampling \n Technique",
    labels=c("SMOTE: \n 100/200","Bootstrap"),
    values=c(rainbow(8)[1],"black")) +
  xlab("1-Specificity") + 
  ylab("Sensitivity") + 
  xlim(0, 1) +
  ylim(0, 1) +
