...
mcols(twointerval)$compartment <- mcols(subcompartgr)$compartment[subjectHits(overlaps2[!duplicated(queryHits(overlaps2))])]
#attaching percentage
mcols(twointerval)$percentA <- ifelse(mcols(twointerval)$compartment=="A",
                                      round(width(ptwointerval[!duplicated(queryHits(overlaps2))])/width(twointerval),2),
                                      round(1-(width(ptwointerval[!duplicated(queryHits(overlaps2))])/width(twointerval)),2))
mcols(twointerval)$percentB <- ifelse(mcols(twointerval)$compartment=="B",
                                      round(width(ptwointerval[!duplicated(queryHits(overlaps2))])/width(twointerval),2),
                                      round(1-(width(ptwointerval[!duplicated(queryHits(overlaps2))])/width(twointerval)),2))
