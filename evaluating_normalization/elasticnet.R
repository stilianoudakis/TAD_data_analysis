...
enetlst <- list(tpr <- matrix(nrow=ceiling((length(which(chr1data_f$y=="Yes"))*2)*.3), 
                             ncol=bootsamps),
...
#set length of list objects that will be filled in with specificities
#and sensitivities and aucs and variable importance
enetlst_nl <- list(tpr <- matrix(nrow=ceiling((length(which(chr1data_f$y=="Yes"))*2)*.3), 
                             ncol=bootsamps),
                fpr <- matrix(nrow=ceiling((length(which(chr1data_f$y=="Yes"))*2)*.3), 
                              ncol=bootsamps),
                auc <- numeric(bootsamps),
                varimp <- matrix(nrow=dim(chr1data_f)[2]-1,
                                ncol=bootsamps))
rownames(enetlst_nl[[4]]) <- colnames(chr1data_f)[-1]
...
for(i in 1:bootsamps){
  ...
  pred.eNetModel <- as.vector(predict(eNetModel, 
                                      newdata=test, 
                                      type="prob")[,"Yes"])
  enetlst_nl[[1]][,i] <- simple_roc(ifelse(test$y=="Yes",1,0),pred.eNetModel)[,1]
  enetlst_nl[[2]][,i] <- simple_roc(ifelse(test$y=="Yes",1,0),pred.eNetModel)[,2]
  enetlst_nl[[3]][i] <- pROC::auc(pROC::roc(test$y, pred.eNetModel))
  enetlst_nl[[4]][,i] <- varImp(eNetModel)$importance[,1]
